...
// React React 相关包（仅在 RN 环境使用，Node.js 不需要）
const external = [
  "react-native",
  "react-native-fs",
...
try {
  buildSync({
    ...
    sourcemap: false,
    banner: {
      js: `// 邮件发送工具 v${version} - 包含所有依赖，无需安装\n`,
    },
    define: {
      '__VERSION': `"${version}"`,
    },
...
console.log("\n使用方式:");
console.log("  node skill.js send_email --to <email> --subject <subject> --body <body>");
console.log("  node skill.js get_config");
console.log("  node skill.js test_smtp_connection");
