#!/usr/bin/env node
// 邮件发送工具 v260224.222700 - 包含所有依赖，无需安装
...
var require_shared = __commonJS({
  "node_modules/.pnpm/nodemailer@6.10.1/node_modules/nodemailer/lib/shared/index.js"(exports2, module2) {
    ...
    module2.exports._logFunc = (logger, level, defaults, data, message, ...args2) => {
      let entry = {};
      ...
      logger[level](entry, message, ...args2);
    };
    module2.exports.getLogger = (options, defaults) => {
      ...
      levels.forEach((level) => {
        response[level] = (data, message, ...args2) => {
          module2.exports._logFunc(logger, level, defaults, data, message, ...args2);
        };
      });
      return response;
    ...
    function createDefaultLogger(levels) {
      ...
      let print = (level, entry, message, ...args2) => {
        let prefix = "";
        if (entry) {
          if (entry.tnx === "server") {
            prefix = "S: ";
        ...
        message = util.format(message, ...args2);
        message.split(/\r?\n/).forEach((line) => {
  ...
  "node_modules/.pnpm/nodemailer@6.10.1/node_modules/nodemailer/lib/smtp-connection/index.js"(exports2, module2) {
    ...
    var SMTPConnection = class extends EventEmitter {
      ...
      _onError(err, type, data, command2) {
        ...
        err = this._formatError(err, type, data, command2);
        this.logger.error(data, err.message);
        this.emit("error", err);
        this.close();
      }
      _formatError(message, type, response, command2) {
        let err;
       ...
       * @private
       */
      _isDestroyedMessage(command2) {
        if (this._destroyed) {
          return "Cannot " + command2 + " - smtp connection is already destroyed.";
        }
        if (this._socket) {
          if (this._socket.destroyed) {
            return "Cannot " + command2 + " - smtp connection socket is already destroyed.";
          }
          if (!this._socket.writable) {
            return "Cannot " + command2 + " - smtp connection socket is already half-closed.";
          }
        }
      }
      _getHostname() {
  ...
  "node_modules/.pnpm/nodemailer@6.10.1/node_modules/nodemailer/lib/ses-transport/index.js"(exports2, module2) {
    ...
    var SESTransport = class extends EventEmitter {
      ...
      _checkRatedQueue() {
        ...
        this._send(next.mail, (...args2) => {
          setImmediate(() => next.callback(...args2));
          this._sent();
        });
      }
      _checkSendingRate() {
...
var SKILL_VERSION = true ? "260224.222700" : "1.0.0-dev";
...
function loadEnv() {
  ...
  if (env2.SMTP_USERNAME && !env2.SMTP_USER) {
    env2.SMTP_USER = env2.SMTP_USERNAME;
  }
  if (env2.SMTP_USE_TLS !== void 0 && env2.SMTP_SECURE === void 0) {
    env2.SMTP_SECURE = env2.SMTP_USE_TLS;
  }
  if (env2.DEFAULT_SENDER_EMAIL && !env2.SMTP_FROM) {
    env2.SMTP_FROM = env2.DEFAULT_SENDER_EMAIL;
  }
  if (env2.DEFAULT_SENDER_NAME && !env2.SMTP_FROM_NAME) {
    env2.SMTP_FROM_NAME = env2.DEFAULT_SENDER_NAME;
  }
  return env2;
}
var env = loadEnv();
...
async function testSmtpConnection() {
  console.log("\n" + "=".repeat(70));
  console.log("\u{1F50C} \u6D4B\u8BD5 SMTP \u8FDE\u63A5");
  console.log("=".repeat(70));
...
async function sendEmail(args2) {
  const requiredKeys = ["SMTP_HOST", "SMTP_PORT", "SMTP_USER", "SMTP_PASSWORD"];
  const missingKeys = requiredKeys.filter((key) => !env[key]);
  if (missingKeys.length > 0) {
    console.error("\n\u274C \u7F3A\u5C11\u5FC5\u9700\u7684\u73AF\u5883\u53D8\u91CF:");
  ...
  if (args2.length > 0 && !args2[0].startsWith("--")) {
    mailConfig = readJsonConfig(args2[0]);
  } else {
    mailConfig = parseArgs(args2);
  }
...
\u670D\u52A1\u5668: ${env.SMTP_HOST}:${env.SMTP_PORT}`);
  ...
  const mailOptions = {
    ...
    cc: config.cc_emails.length > 0 ? config.cc_emails.join(", ") : void 0,
    bcc: config.bcc_emails.length > 0 ? config.bcc_emails.join(", ") : void 0,
